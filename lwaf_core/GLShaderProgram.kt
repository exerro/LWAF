...
class GLShaderProgram(private val programID: Int, private val fragmentID: Int, private val geometryID: Int?, private val vertexID: Int, private val instanced: Boolean): GLResource {
    ...
    fun setUniform(name: String, value: Boolean) {
        Logging.log("shader.uniform") { "Setting shader uniform $name to $value" }
...
private fun loadShader(shaderContent: String, shaderType: Int): Int {
    ...
    Logging.log("shader.compile.notice") { when (shaderType) { GL20.GL_FRAGMENT_SHADER -> "Compiling fragment shader"; GL20.GL_VERTEX_SHADER -> "Compiling vertex shader"; else -> "Compiling geometry shader" } }
    Logging.log("shader.compile.content") { shaderContent }
