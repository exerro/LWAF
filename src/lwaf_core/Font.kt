...
class Font(
    ...
    companion object {
        val DEFAULT_FONT: Font by lazy {
            loadResource("Open Sans") { ->
                loadFontFromInputStream(Font::class.java.getResourceAsStream("/res/open-sans/OpenSans-Regular.fnt"), "Open Sans") {
                    loadTextureFromInputStream(Font::class.java.getResourceAsStream("/res/open-sans/OpenSans-Regular.png"), "Open Sans texture")
                }
            }
        }
    }
...
fun loadFont(filePath: String): Font
        = loadFontFromInputStream(Files.newInputStream(Paths.get(filePath)), filePath, ::loadTexture)

fun loadFontFromInputStream(stream: InputStream, resID: String, textureLoader: (String) -> GLTexture): Font {
    val lineHeight: Int
    val base: Int
    val texture: GLTexture
    ...
    val fileContent = String(stream.readAllBytes())
    ...
    return Font(lineHeight, base, uvs, offsets, sizes, xAdvances, kerning, 1f, "$resID@$lineHeight", texture)
}
