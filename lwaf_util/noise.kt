...
private val perm = ShortArray(512) { i -> p[i and 255] }
private val permMod12 = ShortArray(512) { i -> (perm[i] % 12).toShort() }
...
fun noise(xin: Double, yin: Double): Double {
    ...
    val t = (i + j) * G2
    val x0 = xin - i + t // The x,y distances from the cell origin
    val y0 = yin - j + t
    ...
    if (t0 < 0)
        ...
        t0 *= t0
        n0 = t0 * t0 * dot(grad3[gi0.toInt()], x0, y0)  // (x,y) of grad3 used for 2D gradient
    }
    ...
    if (t1 < 0)
        ...
        t1 *= t1
        n1 = t1 * t1 * dot(grad3[gi1.toInt()], x1, y1)
    }
    ...
    if (t2 < 0)
        ...
        t2 *= t2
        n2 = t2 * t2 * dot(grad3[gi2.toInt()], x2, y2)
    }
...
fun noise(xin: Double, yin: Double, zin: Double): Double {
    ...
    val t = (i + j + k) * G3
    val x0 = xin - i + t // The x,y,z distances from the cell origin
    val y0 = yin - j + t
    val z0 = zin - k + t
    ...
    if (t0 < 0)
        ...
        t0 *= t0
        n0 = t0 * t0 * dot(grad3[gi0.toInt()], x0, y0, z0)
    }
    ...
    if (t1 < 0)
        ...
        t1 *= t1
        n1 = t1 * t1 * dot(grad3[gi1.toInt()], x1, y1, z1)
    }
    ...
    if (t2 < 0)
        ...
        t2 *= t2
        n2 = t2 * t2 * dot(grad3[gi2.toInt()], x2, y2, z2)
    }
    ...
    if (t3 < 0)
        ...
        t3 *= t3
        n3 = t3 * t3 * dot(grad3[gi3.toInt()], x3, y3, z3)
    }
    // Add contributions from each corner to get the final noise value.
